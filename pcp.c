...
void grow_mots(){
        taille_mot*=2;
        mot_haut = (unsigned char*)realloc(mot_haut,sizeof(unsigned char)*taille_mot);
        mot_bas = (unsigned char*)realloc(mot_bas,sizeof(unsigned char)*taille_mot);
}
...
int choose_domino(){
        ...
        for(j=0;j<doms_lus;j++){
                if(doms_mask[j]>0){ nonnul=j;count++;}
        }
        if(count == 1){
...
int try_domino(int dom){
        ...
        if(strncmp((char*)mot_haut,(char*)mot_bas, minimum(pos_haut+strlen((char*)domsu[dom]), pos_bas + strlen((char*)domsv[dom]))) == 0) ret = 1;
        mot_haut[pos_haut] = 0;
        mot_bas[pos_bas]=0;
        return ret;
}
